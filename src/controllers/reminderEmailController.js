    ...
    try {
        ...
        if (machines.length === 0) {
            return res.status(404).json({
                success: false,
                message: 'No machines are due for renewal within the next 45 days.'
            });
        }
        ...
        const {
            ...
            error,
            messageId
        } = await sendEmail(emailAddress, 'Terminal Renewal Reminder', emailBody, 'Due Date Reminder', [ process.env.CS_EMAIL ]);

        updateEmail(insertedEmail.lastInsertRowid, {
            status: ok ? 'sent' : 'failed',
            nodemailer_message_id: messageId,
            failed_reason: error ? (error.message || 'Unknown error') : null
        });
        ...
        if (notRemindedMachines.length > 0) {
            ...
            for (const machine of notRemindedMachines) {
                try {
                    ...
                    logger.error(`Error occurred when inserting process renewal id to freshly sent email in manual send email. \
            ...
            if (errorMachines.length > 0) {
                ...
                const {
                    ok,
                    error
                } = await sendEmail(process.env.DEV_EMAIL, '|AR VENDING| Error occured in inserting renewal process id after sending email manually.', emailBody2);
                if (!ok) {
                    logger.error(`Failed to notify to developer on renewal process id insertion failure in manual email reminder.`);
                }
                return res.status(500).json({
                    success: false,
                    message: 'Email is sent to the customer. However, there was an error when performing database operation. Notify the developer.'
                });
            }
        }
        ...
        return res.status(500).json({
            success: false,
            message: `Failed to send reminder email. Please try again later. Error: ${error.message}`
        });
    }
}
